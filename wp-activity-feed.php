      ...
      foreach ( (array) $items as $item ){
        ...
        switch ($item->act_type){
          case 'CONNECT':
            ...
            $item_desc = '<a href="'.$wp_url.'/author/'.$user_nicename.'" title="'.__('View Profile', 'wp-activity').'">'.$users_display[$item->user_id].'</a> '.__('has logged.', 'wp-activity');
            break;
          case 'COMMENT_ADD':
            ...
            $item_desc = '<a href="'.$wp_url.'/author/'.$user_nicename.'" title="'.__('View Profile', 'wp-activity').'">'.$item_comment->comment_author.'</a> '.__('commented', 'wp-activity').' <a href="'.$item_post->post_name.'#comment-'.$item_comment->comment_ID.'">'.$item_post->post_title.'</a>';              
            break;
          case 'POST_ADD':
            $flag = ($act_feed_posts) ? true : false ;
            $item_title = __('New post', 'wp-activity');
            $item_post=get_post($item->act_params);
            $item_desc = '<a href="'.$wp_url.'/author/'.$user_nicename.'" title="'.__('View Profile', 'wp-activity').'">'.$users_display[$item_post->post_author].'</a> '.__('published', 'wp-activity').' <a href="'.$item_post->post_name.'">'.$item_post->post_title.'</a>';
            break;
          case 'POST_EDIT':
            $flag = ($act_feed_posts) ? true : false ;
            $item_title = __('Post edited', 'wp-activity');
            $item_post=get_post($item->act_params);
            $item_desc = '<a href="'.$wp_url.'/author/'.$user_nicename.'" title="'.__('View Profile', 'wp-activity').'">'.$users_display[$item_post->post_author].'</a> '.__('edited', 'wp-activity').' <a href="'.$item_post->post_name.'">'.$item_post->post_title.'</a>';
            break;
          case 'PROFILE_EDIT':
            $flag = ($act_feed_profiles) ? true : false ;
            $item_title = __('Profile edited', 'wp-activity');
            $item_desc = '<a href="'.$wp_url.'/author/'.$user_nicename.'" title="'.__('View Profile', 'wp-activity').'">'.$users_display[$item->user_id].'</a> '.__('has updated his profile.', 'wp-activity');
            break;
          case 'LINK_ADD':
            ...
            if ($link->link_visible == 'Y'){
              $item_desc = '<a href="'.$wp_url.'/author/'.$user_nicename.'" title="'.__('View Profile', 'wp-activity').'">'.$users_display[$item->user_id].'</a> '.__('has added a link to', 'wp-activity').' <a href="'.$link->link_url.'" title="'.$link->link_description.'" target="'.$link->link_target.'">'.$link->link_name.'</a>.';
            }
            break;
        }
        if ($flag){
          ...
          $cache .='<dc:creator>'.$users_display[$item->user_id].'</dc:creator>';
          $cache .='<link>'.$wp_url.'</link>';
          $cache .='</item>';
        }
      }
